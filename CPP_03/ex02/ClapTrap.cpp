...
/*   Updated: 2024/03/14 13:32:52 by ehouot           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ClapTrap.hpp"

ClapTrap::ClapTrap( void ) : _healthStart(10), _name("NoName"), _health(10), _energy(10), _attack(0)
{
	std::cout << "Default constructor called" << std::endl;
}

ClapTrap::ClapTrap( std::string name) : _healthStart(10), _name(name), _health(10), _energy(10), _attack(0)
{
	std::cout << "Parametric constructor called" << std::endl;
}

ClapTrap::ClapTrap( const ClapTrap &src ) : _healthStart(src._healthStart), _name(src._name), _health(src._health), _energy(src._energy), _attack(src._attack)
{
...
void	ClapTrap::setHealthStart( int healthStart)
{
	this->_healthStart = healthStart;
}
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		std::cout << "ClapTrap " << this->getGlobal().name << " is already dead." << std::endl << std::endl;
		return ;
	}
	...
	std::cout << "ClapTrap " << this->getGlobal().name << " has now " << this->getGlobal().health << " HP." << std::endl;
	if (this->getGlobal().health <= 0)
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (this->getGlobal().health >= this->getGlobal().healthStart)
	...
	else
	{
		...
		setEnergy(this->_energy - 1);
		if (this->getGlobal().health + amount > (unsigned int)this->getGlobal().healthStart)
		{
			realAmount = this->getGlobal().healthStart - this->getGlobal().health;
			setHealth(this->_health + realAmount);
		}
		else
			setHealth(this->_health + amount);
